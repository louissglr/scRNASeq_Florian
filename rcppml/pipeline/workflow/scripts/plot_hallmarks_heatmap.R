...
enrich_df <- read.csv(input_csv_enrichment, stringsAsFactors = FALSE)
...
cat("Nombre de hallmarks significatifs :", nrow(heatmap_df), "\n")

heatmap_mat <- heatmap_df %>%
  column_to_rownames("gene.set") %>%
  as.matrix()

colnames(heatmap_mat) <- str_replace(colnames(heatmap_mat), "^program_", "")
...
pdf(output_pdf_enrich, width = 8, height = 10)
draw(ht)
dev.off()

cat("Heatmap exportée dans :", output_pdf_enrich, "\n")